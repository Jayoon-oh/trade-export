...
import { getPackingList, updatePackingList } from '../../../api/packingListApi';
...
interface PackingListEditFormProps {
    packingListId: number;
    onSuccess: () => void;
}

function PackingListEditForm({ packingListId, onSuccess }: PackingListEditFormProps) {
    ...
    const fetchDetail = async () => {
        ...
        setForm({
            ...
            items: detail.items.map((item) => ({ itemsId: item.itemsId, quantity: item.quantity, actualWeight: item.actualWeight }))
        });
    };
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">패킹리스트 수정</h2>
